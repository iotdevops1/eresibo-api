...
class DocumentVaultDocumentResource extends JsonResource {
    ...
    {
        $scope = $request->is('api/admin/*') ? 'admin' : ($request->is('api/employer/*') ? 'employer' : 'employee');
        $sourceUrl = $scope === 'employee' && $this->document_type === DocumentVaultDocument::TYPE_PAYSLIP
            ? url('/api/employee/payslips/'.$this->source_uuid)
            : null;
        ...
        return [
            ...
            'links' => ['self' => url('/api/'.$scope.'/documents/'.$this->uuid), 'source' => $sourceUrl],
